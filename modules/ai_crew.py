...
def get_most_discounted_books(n: int = 5) -> List[Dict]:
    ...
    Returns:
        List[Dict]: A list of dictionaries containing book details.
    """
    return df.nlargest(n, "discount_percentage")[
        [
            "Book-Title",
            "Book-Author",
            "Year-Of-Publication",
...
class SingleExtractedBook(BaseModel):
    ...
    discount_percentage: float = Field(
        title="The discount percentage on the book. Set to None if no discount",
        default=None,
    )
    price_after_discount: float = Field(
        title="The final price after applying the discount. Set to None if no discount",
        default=None,
    )
    ...
    agent_recommendation_rank: int = Field(
        ...,
        title="The book's rank in the recommendation list (out of 5, Higher is Better)",
    )
    agent_recommendation_notes: List[str] = Field(
        ..., title="Reasons why this book is recommended or not compared to others"
    )
...
class Consultantion(BaseModel):
    personalized_sales_assistance: str = Field(
        ..., title="Personalized sales assistance"
    )
    book_recommendations: AllExtractedBooks = Field(
        title="Book recommendations based on user preferences", default=None
    )
...
ai_crew = Crew(
    agents=[manager_agent, data_agent, sales_consultant_agent],
    tasks=[manager_task, data_task, sales_consultant_task],
